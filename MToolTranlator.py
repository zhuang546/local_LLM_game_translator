...
'''
class MToolTranlator(ConfigManager):
    def __init__(self):
        self.config_path = 'MToolTranlator_config.ini'
        self.messages_path = 'MToolTranlator_messages.json'

        super().__init__(self.config_path, self.messages_path, default_config) # 调用父类初始化配置，这里传入了一个不同的默认配置
        self.group_length = self.config.getint('Extra', 'group_length', fallback=default_config['Extra']['group_length']) # 分组长度
    ...
    def translate_json(self):
        ...
        for index, group in enumerate(groups):
            # 检查是否已存在翻译文件
            temp_filepath = MToolTranlator.get_temp_file_path(self.file_path, index+1)
            ...
            for key, text in tqdm(group.items(), desc=f'翻译第{index+1}批文本'):
                if not ascii_only_pattern.match(text):
        ...
        # 输出错误生成的文本到文件
        if error_texts:
            error_file_path = MToolTranlator.get_temp_file_path(self.file_path, 'error')
