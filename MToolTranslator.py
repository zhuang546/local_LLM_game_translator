...
from pathlib import Path as pathlib # 路径兼容性
...
class MToolTranslator(ConfigManager):
    ...
    def __init__(self):
        self.config_path = pathlib(path.split(argv[0])[0],'MToolTranslator_config.ini')
        self.messages_path = pathlib(path.split(argv[0])[0],'MToolTranslator_messages.json')
    ...
    def get_temp_file_path(file_path, index):
        # 生成临时文件路径
        temp_folder = pathlib(path.dirname(file_path), 'Temp Files')
        if not path.exists(temp_folder):
            mkdir(temp_folder)
        result = pathlib(temp_folder, path.splitext(path.basename(file_path))[0]+f'_translated_{index}.json')
        return result
    ...
    def translate_json(self):
        ...
        for index, group in enumerate(groups):
            ...
            for key, text in tqdm(group.items(), desc=f'翻译第{index+1}批文本'):
                if not MToolTranslator.ascii_only_pattern.match(key):
                    translated_texts[key] = self.client.translate(key) 
                else:
        ...
        if error_texts:
            for key, value in error_texts.items():
                retranslated = self.client.translate(key)
                translated_texts[key] = retranslated
                value = f'{retranslated}\n【原文：{value}】'
    ...
    def run(self, argv=[]):
        # 通过run函数启动翻译流程
        if  len(argv) > 1:
            self.file_path = pathlib(argv[1]) # 从命令行参数获取文件名
            print('已获取文件路径：', self.file_path)
        else:
            self.file_path = pathlib(input('请输入待翻译文件的路径（可带引号）：').strip('\'" ')) # 手动输入文件路径
