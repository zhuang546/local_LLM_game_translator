...
class UnityTranslator(ConfigManager):
    def __init__(self):
        ...
        super().__init__(self.config_path, self.messages_path, default_config)
        self.system_message_template = self.config.get('Extra', 'system_message_template', fallback=default_config['Extra']['system_message_template']) # 用于系统消息更新的模板
        ...
        def translate():
            ...
            if not from_lang == 'en' and ascii_only_pattern.match(text): # 如果源语言不是英语且文本全是单字节字符，直接返回文本
                return text
